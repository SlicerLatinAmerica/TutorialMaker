...
class markdownHTMLCreator:
    ...
    def ensure_pdfkit_installed(self):
        try:
            ...
            #print('PDFKit ya esta instalado')
        except ImportError:
    ...
    def tutorial_to_markdown(self,path,ListotalImages):
        ...
        with open(path + ".md", 'w', encoding='utf-8') as md_file:
           ...
           # md_file.write(f'  <div class="header" style="background-color: #a8d0e6; padding: 20px; color: white; font-size: 40px; font-weight: bold; text-align: center; max-width: 90%; margin: 0 auto;">Slicer4 Minute</div>\n')
           # md_file.write(f'<br>\n')
           # md_file.write(f'<br>\n')
           # md_file.write(f'  <div class="subheader" style="font-size: 30px; margin: 10px 0; color: gray; text-align: center; max-width: 90%; margin: 0 auto;">Sonia Pujol, Ph.D.</div>\n')
           # md_file.write(f'  <div class="subheader" style="font-size: 30px; margin: 10px 0; color: gray; text-align: center; max-width: 90%; margin: 0 auto;">Assistant Professor of Radiology</div>\n')
           # md_file.write(f'  <div class="subheader" style="font-size: 30px; margin: 10px 0; color: gray; text-align: center; max-width: 90%; margin: 0 auto;">Brigham and Women’s Hospital</div>\n')
           # md_file.write(f'  <div class="subheader" style="font-size: 30px; margin: 10px 0; color: gray; text-align: center; max-width: 90%; margin: 0 auto;">Harvard Medical School</div>\n')
           # md_file.write('</div>\n')

            # Footer section
            #md_file.write(f'<div class="containerFooter" style="text-align: center; width: 100%;">\n')
           # md_file.write(f'<img class="LineImage" src="{os.path.dirname(slicer.util.modulePath("TutorialMaker"))}/Resources/Icons/Painter/line_down.png" style="width: 100%; margin-bottom: 10px;">\n')
           # md_file.write(f'</div>\n')
                ...
                if (ListotalImages[num] == -1):
                    if (num == 0):
                       ...
                       # Header section
                        ...
                        authors = metadatos[item]["slide_text"].replace('\n', '<br>')
                        if authors.endswith('<br>'):
                            authors = authors[:-4]  # Quita los últimos 4 caracteres ("<br>")
                        ...
                        # Slide Title
                        md_file.write(f'<div class="titulo" style="margin: 20px auto;"><h2>{metadatos[item]["slide_title"]}</h2></div>\n\n')

                        # White container with text
                        md_file.write(f'<div class="containerWhite" style="background-color: #ffffff; padding: 20px; margin: 20px auto; max-width: 90%;">\n')
                        md_file.write(f'<div class="textWhite" style="font-size: 18px;">{metadatos[item]["slide_text"]}</div>\n')
                    ...
                    # Slide Text
                    md_file.write(f'<div class="text" style="font-size: 18px; margin: 10px auto; max-width: 90%; text-align: center;">{metadatos[item]["slide_text"]}</div><br>\n\n')
    ...
    def markdown_to_html(self,path, ListTotalImages):
        ...
        confirm = qt.QMessageBox.question(slicer.util.mainWindow(), "HTML Gene  rated", message,
                                           qt.QMessageBox.Yes | qt.QMessageBox.No)
        if confirm == qt.QMessageBox.Yes:
    ...
    def html_to_pdf(self,path, output_html_file, tutorialName):
        ...
        print(f"Ruta del archivo HTML: {output_html_file}")
        # Verificar la existencia del archivo HTML
        try:
            pdfkit.from_file(output_html_file, output_pdf_file, options=options, configuration=config)
            message = "PDF file '{}' has been generated. Would you like to open it?".format(tutorialName)
            confirm = qt.QMessageBox.question(slicer.util.mainWindow(), "Markdown Generated", message,
                                            qt.QMessageBox.Yes | qt.QMessageBox.No)
            if confirm == qt.QMessageBox.Yes:
