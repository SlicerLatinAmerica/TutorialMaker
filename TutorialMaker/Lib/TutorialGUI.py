                ...
                try:
                    if joinedImage == None:
                        ...
                        with open(path_meta, 'r', encoding='utf-8') as fileP:
                            joinedJson.update(json.loads(fileP.read()))

                        jsonStringA = json.dumps(joinedJson)
                        painter = qt.QPainter(joinedImage)
                        windowImage = qt.QImage(path_image)
                        painter.drawImage(qt.QRect(windowJson[0]["position"][0], windowJson[0]["position"][1], windowImage.width(), windowImage.height()), windowImage)
                        painter.end()
                    ...
                    print(_("An unexpected error occurred while opening file '{path_image}': {error}".format(path_image=path_meta, error=str(e))))
                    exception_occurred = True
                    break
